...
<body>
  ...
  const appProps = {
    ...
    onResultClick: (result: any) => {
      setFocusTarget({ lat: result.lat, lon: result.lon, zoom: 12, id: result.id, label: result.label });
      setSelectedId(result.type === 'incident' ? result.id : null);
    },
    largeLabels,
    setLargeLabels,
    allLabels,
    ...
    pauseRecording: () => { setRecordingStatus('paused'); },
    resumeRecording: () => { setRecordingStatus('recording'); },
    stopRecording: hookStopRecording,
    recordingToRoute: hookRecordingToRoute,
    saveRecording: hookSaveRecording,
    navStartId,
    setNavStartId,
    navEndId,
    setNavEndId,
